...
class PayModeController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validate = Validator::make($request->all(), [
            'name' => ['required', 'max:50', 'unique:pay_mode,name'],
    ...
    public function show($id)
    {
        ...
        if (is_null($payMode)) {
            return response()->json(['msg' => 'Modo de pago no encontrado.'], 404);
        }
    ...
    {
        $validate = Validator::make($request->all(), [
            'name' => ['required', 'max:50', 'unique:pay_mode,name,' . $id],
            'observation' => ['nullable', 'max:200'],
        ]);
    ...
    public function destroy($id)
    {
        ...
        return response()->json(['pay_mode' => $payModes]);
    }
}
